...
export default function BuyerDashboard({ onEnquireAdmin }) {
  ...
  // Live listener for opening specific car details from AI Chatbot or external links
  useEffect(() => {
    const handleOpenDetails = (e) => {
      const carId = String(e?.detail?.carId || '');
      if (carId) {
        const found = allCars.find(c => String(c.id) === carId || String(c._id) === carId);
        if (found) {
          setViewingCar(found);
        } else {
          api.getCarById(carId).then(car => {
            if (car) setViewingCar(car);
          }).catch(() => {});
        }
      }
    };

    window.addEventListener('carhub_open_car_details', handleOpenDetails);
    return () => window.removeEventListener('carhub_open_car_details', handleOpenDetails);
  }, [allCars]);
